...
def index(request):
    return HttpResponse("Hola! esta es la página principal de Mi tienda!") #es dinamico

#hay que asociarle una URL
...
def test1(request):
    ...
    # Párrafo a insertar
    P = "<p>Numero aleatorio: " + str(numero) + " </p>" #PAGINA AL VUELO

    PAGINA_INI = """
    <!DOCTYPE html>
    <html lang="es" dir="ltr">
      <head># -- Ejemplo de generacion mediante una plantilla en FICHERO
def test3(request):
...
def test2(request):
    ...
    # --Procesar la plantilla, a django
...
def list(request):
    productos = Producto.objects.all()#leer todos los objetos de tipo producto
